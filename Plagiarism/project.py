...
#FOLDER PATH
UPLOAD_FOLDER ='/home/sarthak/Desktop/Project/Plagiarism'
...
name_list = []
selectedFile=[]
dead_line = datetime.datetime(2025,5,2,11,30)
		        ...
		        if con.execute("SELECT * FROM USER_DATA WHERE password = ? ",[password1]).fetchone():      
		            ...
		            if con.execute("SELECT * FROM USER_DATA WHERE file_Name = ? ",[file_name]).fetchone(): 
		               ...
		               #storing file_name into USER_DATA
		                ...
		                con.execute("Update USER_DATA set file_Name=? where username=?",(file_name,a))
		                #file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
...
############################################################################################################
    ...
    def CSlsa():
        if request.method == 'POST':
            ...
            src=fName[0][0]
            dest=selectedFile[0]
            photo_list=gb.glob('/home/sarthak/Desktop/Project/integrated_flask/static/*.jpeg')
            for k in photo_list:
                if os.path.exists(k):
                    os.remove(k)
            #print("CSALSA\n",src,"  ",dest)
