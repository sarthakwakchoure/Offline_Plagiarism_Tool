...
def newColBOW(Doc):
    #appending words of Uploaded Doc in BAG_OF_WORDS_LIST if not present 
    for word in Doc :
...
def wordPresentChecker(Doc):
    #if word from BAG_OF_WORDS_LIST is present in Uploaded Doc then that word(key) will be assigned with 1(value) else will be assigned with 0(value) 
    j=0
